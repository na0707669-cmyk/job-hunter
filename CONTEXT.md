...
- [x] **필체 샘플 기반 자소서 작성** — 작성자 샘플 글을 넣으면 문장 길이·전개 방식·말투 습관을 반영해 초안 생성
...
/api/draft                 자소서 초안 생성 — style_sample 선택 입력 지원
/api/job-detail            공고 원문 일부 스크래핑 → 앱 내부 상세 모달 표시
...
- [x] **자소서 첨삭** — 생성/저장된 초안에 대해 논리성·구체성·직무적합도·문장력 피드백과 수정 예시 제공 (`/api/draft/critique`, 드로어 내 ✏️ 버튼)
